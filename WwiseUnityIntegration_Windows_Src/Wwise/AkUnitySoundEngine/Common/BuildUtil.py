...
DefaultLogPath = normpath(join(ScriptDir, Parent, Parent, 'Logs'))
DefaultLogFile = normpath(join(DefaultLogPath, '{0}_{1}.log'.format(MetadataBaseName, (datetime.now() - datetime(1970, 1, 1)).total_seconds())))
IsVerbose = False
...
# Unity platform vs. Wwise native bank platforms (WwiseConsole --platform switch).
BankPlatforms = \
{
...
PlatformSwitches = \
{
	'Common': '#if !(UNITY_QNX) // Disable under unsupported platforms.'
}

PlatformDependentFilenames = \
[
	'AkUnitySoundEngine.{}'.format(ApiExt),
	'AkUnitySoundEnginePINVOKE.{}'.format(ApiExt),
...
class PathManager(object):
	def __init__(self, PlatformName, Arch=None, wwiseSdkRootDir=None):
		...
		self.Paths['Log'] = normpath(join(DefaultLogPath, '{0}_{1}_{2}.log'.format(MetadataBaseName, PlatformName, datetime.now().strftime('%Y-%m-%d_%H.%M.%S'))))
		self.logger = CreateLogger(self.Paths['Log'], __file__, self.__class__.__name__)

		if PlatformName in ['Android', 'OpenHarmony'] and SpecialChars['WhiteSpace'] in self.Paths['Root']:
			msg = ('Unity Integration root path contains white spaces. Android/OpenHarmony build will fail.'
				   'Consider removing all white spaces in Unity Integration root path: {}. Aborted.').format(
				self.Paths['Root'])
		...
		self.ProductName = 'AkUnitySoundEngine'
		...
		self.Paths['Wwise_Root'] = self.NormJoin(self.Paths['Wwise_SDK'], Parent)
		self.Paths['Toolchain_setup'] = self.NormJoin(self.Paths['Wwise_Root'], 'Scripts', 'ToolchainSetup')
		self.Paths['Premake_Scripts'] = self.NormJoin(self.Paths['Wwise_Root'], 'Scripts', 'Premake')
		...
		if sys.platform == 'darwin':
			if "akm" in socket.gethostname().lower():
				self.Paths['SWIGEXE'] = str((Path.home() / 'homebrew' / 'bin' / 'swig').resolve())
				self.Paths['SWIG_CSharpModule'] = str((Path.home() / 'homebrew' / 'opt' / 'swig' / 'share' / 'swig' / '4.3.0' / 'csharp').resolve())
			else:
				self.Paths['SWIGEXE'] = '/usr/local/bin/swig'
				self.Paths['SWIG_CSharpModule'] = '/usr/local/Cellar/swig/4.3.0/share/swig/4.3.0/csharp'

			self.Paths['SWIG_WcharModule'] = self.NormJoin(self.Paths['SWIG_CSharpModule'], 'wchar.i')
			self.Paths['Premake_exe'] = self.NormJoin(self.Paths['Wwise_Root'], 'Tools', 'Mac', 'bin', 'premake5')
		elif sys.platform == 'linux':
			self.Paths['SWIGEXE'] = '/usr/local/bin/swig'
			self.Paths['SWIG_CSharpModule'] = '/usr/local/share/swig/4.3.0/csharp'
			self.Paths['SWIG_WcharModule'] = self.NormJoin(self.Paths['SWIG_CSharpModule'], 'wchar.i')
			self.Paths['Premake_exe'] = self.NormJoin(self.Paths['Wwise_Root'], 'Tools', 'Linux', 'bin', 'premake5')
		else:
			self.Paths['SWIG'] = 'c:\\swigwin-4.3.0'
			...
			self.Paths['SWIG_WcharModule'] = self.NormJoin(self.Paths['SWIG_CSharpModule'], 'wchar.i')
			self.Paths['Premake_exe'] = self.NormJoin(self.Paths['Wwise_Root'], 'Tools', 'Win32', 'bin', 'premake5.exe')

		self.Paths['Premake5_lua_file'] = self.NormJoin(self.Paths['Src_Common'], 'premake5.lua')
...
def GetSupportedArches(platform):
	if not(platform in SupportedArches):
		return "Architecture for {} not found".format(platform)
	return SupportedArches[platform]
...
def Init(argv=None):
	...
	parser.add_argument('-a', '--arch', nargs=1, dest='arch', default=None,
						help='The target architecture to build for certain platforms, available options:'
							 'Windows: {}, Android: {}; default to None if none given.'.format(
							', '.join(GetSupportedArches('Windows')), ', '.join(GetSupportedArches('Android'))))
	Undefined = 'Undefined'
	envWwiseSdkDir = Undefined
	if WwiseSdkEnvVar in os.environ:
		envWwiseSdkDir = os.environ[WwiseSdkEnvVar]
	parser.add_argument('-w', '--wwisesdkdir', nargs=1, dest='wwiseSdkDir', default=None,
						help='The Wwise SDK folder to build the Integration against. Required if -u is used.'
							 'Abort if the specified path is not found; if no path specified in arguments,'
							 'fall back to the environment variable WWISESDK. Current WWISESDK = {}.'
							 'For Android/OpenHarmony, no white spaces are allowed in the Wwise SDK folder path.'.format(
							DefaultPrefFile, envWwiseSdkDir))
